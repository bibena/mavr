...
//---send query
			if(in_array('query',$data))
				...
				if ($request->execute($support_array))
					{
					$return=true;
					}
				else
			...
			if(isset($data['where']))
				...
				if(is_array($data['where'][0]))
					...
					foreach($data['where'] as $where)
						{
						$sql.=" `".$where[0]."`".$where[1].":".$where[0]." AND";
						$support_array[":".$where[0]]=$where[2];
						}
					$sql=preg_replace('/AND$/',' ',$sql);
					}
				else
					{
					$sql.=" `".$data['where'][0]."`".$data['where'][1].":".$data['where'][0]." ";
					$support_array[":".$data['where'][0]]=$data['where'][2];
					}
				}
//---ORDER BY claude
			if(isset($data['order_by']) && is_array($data['order_by']))
				{
				$sql.='ORDER BY `'.$data['order_by'][0].'` '.$data['order_by'][1];
				}
//---LIMIT claude
			if(isset($data['limit']))
				{
...
//---GROUP BY claude
			if(isset($data['group_by']) && is_array($data['group_by']))
				{
				$sql.='GROUP BY `'.$data['group_by'][0].'` '.$data['group_by'][1];
				}
//---ORDER BY claude
			if(isset($data['order_by']) && is_array($data['order_by']))
				{
...
* 												)
* 											)
* 							['join'=>array('sql'=>'SELECT*FROM','join'=>'inner','existed_table'=>'tablename1','existed_field'=>'field','added_table'=>'tablename2','added_field'=>'field')]
* 							'where'=>array(
...
//---fields claude
			if(isset($data["fields"]) && is_array($data["fields"]))
				{
				if(isset($data["fields"][0]) && is_array($data["fields"][0]))
					{
					...
					if(isset($data["join"]) && is_array($data["join"]))
						{
						$sql.='`'.$data["fields"]["tablename"].'`.`'.implode('`,`'.$data["fields"]["tablename"].'`.`',$data["fields"]["fields"]).'`';
						}
					else
						{
						$sql.='`'.implode('`,`',$data["fields"]).'`';
...
//---JOIN claude
			if(isset($data["join"]) && is_array($data["join"]))
				{
				if(isset($data["join"][0]) && is_array($data["join"][0]))
					{
					foreach($data["join"] as $join)
						{
						if(isset($join["sql"]))
							{
							$sql.=(isset($join["join"])?' '.$join["join"]:'').' JOIN ('.$join["sql"]["sql"].') AS `'.$join["added_table"].'` ON(`'.$join["existed_table"].'`.`'.$join["existed_field"].'`=`'.$join["added_table"].'`.`'.$join["added_field"].'`) ';
							$support_array=array_merge($support_array,$join["sql"]["support_array"]);
							}
						else
							{
							$sql.=(isset($join["join"])?' '.$join["join"]:'').' JOIN `'.$join["added_table"].'` ON(`'.$join["existed_table"].'`.`'.$join["existed_field"].'`=`'.$join["added_table"].'`.`'.$join["added_field"].'`) ';
							}
						}
					$sql=preg_replace('/\,$/',' ',$sql);
					}
					...
					{
					if(isset($data["join"]["sql"]))
						{
						$sql.=(isset($data["join"]["join"])?' '.$data["join"]["join"]:'').' JOIN ('.$data["join"]["sql"]["sql"].') AS `'.$data["join"]["added_table"].'` ON(`'.$data["join"]["existed_table"].'`.`'.$data["join"]["existed_field"].'`=`'.$data["join"]["added_table"].'`.`'.$data["join"]["added_field"].'`) ';
						$support_array=array_merge($support_array,$data["join"]["sql"]["support_array"]);
						}
					else
						{
						$sql.=(isset($data["join"]["join"])?' '.$data["join"]["join"]:'').' JOIN `'.$data["join"]["added_table"].'` ON(`'.$data["join"]["existed_table"].'`.`'.$data["join"]["existed_field"].'`=`'.$data["join"]["added_table"].'`.`'.$data["join"]["added_field"].'`) ';
						}
					}
				}
//---WHERE claude
			if(isset($data["where"]))
				{
				$sql.='WHERE';
				if(isset($data["where"][0]) && is_array($data["where"][0]))
					{
					foreach($data["where"] as $where)
						{
						if(isset($data["join"]) && is_array($data["join"]))
							{
							$sql.=' `'.$where["tablename"].'`.`'.$where["field"].'`'.$where["symbol"].':'.$where["tablename"].'_'.$where["field"].' AND';
							$support_array[':'.$where["tablename"].'_'.$where["field"]]=$where["value"];
							}
						else
							{
							$sql.=' `'.$where["field"].'`'.$where["symbol"].':'.$where["field"].' AND';
							$support_array[':'.$where["field"]]=$where["value"];
							}
						}
					$sql=preg_replace(' /AND$/','',$sql);
					}
					...
					if(isset($data["join"]) && is_array($data["join"]))
						{
						$sql.=' `'.$data["where"]["tablename"].'`.`'.$data["where"]["field"].'`'.$data["where"]["symbol"].':'.$data["where"]["tablename"].'_'.$data["where"]["field"].' ';
						$support_array[':'.$data["where"]["tablename"].'_'.$data["where"]["field"]]=$data["where"]["value"];
						}
					else
						{
						$sql.=' `'.$data["where"]["field"].'`'.$data["where"]["symbol"].':'.$data["where"]["field"].' ';
						$support_array[':'.$data["where"]["field"]]=$data["where"]["value"];
						}
					}
				}
//---GROUP BY claude
			if(isset($data["group_by"]) && is_array($data["group_by"]))
				...
				if(isset($data["join"]) && is_array($data["join"]))
					{
					$sql.='GROUP BY `'.$data["group_by"]["tablename"].'`.`'.$data["group_by"]["field"].'` ';
					}
				else
					{
					$sql.='GROUP BY `'.$data["group_by"]["field"].'` ';
					}
				}
//---HAVING claude
			if(isset($data["having"]) && is_array($data["having"]))
				{
				$sql.='HAVING';
				if(isset($data["having"][0]) && is_array($data["having"][0]))
					{
					foreach($data["having"] as $having)
						{
						if(isset($having["sql"]))
							{
							$sql.=' '.$having["sql"].' ';
							...
							if(isset($data["join"]) && is_array($data["join"]))
								{
								$sql.=' `'.$having["tablename"].'`.`'.$having["field"].'`'.$having["symbol"]."'".$having["value"]."' AND";
								}
							else
								{
								$sql.=' `'.$having["value"].'`'.$having["field"]."'".$having["value"]."' AND";
								}
							}
						}
					$sql=preg_replace('/AND$/','',$sql);
					...
					if(isset($data["having"]["sql"]))
						...
						if(isset($data["join"]) && is_array($data["join"]))
							{
							$sql.=' `'.$data["having"]["tablename"].'`.`'.$data["having"]["field"].'`'.$data["having"]["symbol"]."'".$data["having"]["value"]."' ";
							}
						else
							{
							$sql.=' `'.$data["having"]["value"].'`'.$data["having"]["field"]."'".$data["having"]["value"]."' ";
							}
						}
					}
				}
//---ORDER BY claude
			if(isset($data["order_by"]) && is_array($data["order_by"]))
				...
				if(isset($data["join"]) && is_array($data["join"]))
					{
					$sql.='ORDER BY `'.$data["order_by"]["tablename"].'`.`'.$data["order_by"]["field"].'`'.(isset($data["order_by"]["sort"])?' '.$data["order_by"]["sort"]:'');
					}
				else
					{
					$sql.='ORDER BY `'.$data["order_by"]["field"].'`'.(isset($data["order_by"]["sort"])?' '.$data["order_by"]["sort"]:'');
					}
				}
//---LIMIT claude
			if(isset($data["limit"]))
...
//---send query
			if(in_array('query',$data))
				{
				$return=array("sql"=>substr($sql,0,-1),"support_array"=>$support_array);
				}
			else
				{
				$request=$db->prepare($sql);
