...
/*****************************************************************************************************************
 * 
 * Functions to administrate user setting
 * 
*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function Admin_User_Save($form)
		{
		try
			{
			global $db,$sql;
			if($form["id"]>0)
				{
//---update name,description,price,amount,visibility,modification_time
				$updating_array=array("email"=>$form["email"],
										"first_name"=>$form["first_name"],
										"last_name"=>$form["last_name"],
										"country_id"=>$form["country_id"],
										"region_id"=>(isset($form["region_id"]) && $form["region_id"])?$form["region_id"]:null,
				...
				if(isset($form["password"]) && preg_match("|^[-a-z0-9!#$%&'*+/=?^_`{\|}~]{4,}$|i",$form["password"]))
					{
					require_once(ROOT_DIR.'/model/user/password.php');
					$updating_array["password"]=password_hash($form["password"],PASSWORD_BCRYPT,array("cost"=>12));
					}
						...
						if(strpos($form["postfiles"]["images"]["type"][$sortid],'image')!==false)
							...
							$sql->Insert(array("tablename"=>'images',
												"set"=>array("image_name"=>$form["postfiles"]["images"]["name"][$sortid],
															...
															"is_visible"=>$imagedata["visible"])));
							$lastid=$db->lastInsertId();
							if(!file_exists(ROOT_DIR.DS.'image'.DS.'users'.DS.$form["id"]))
								{
								mkdir(ROOT_DIR.DS.'image'.DS.'users'.DS.$form["id"]);
								}
							rename(ROOT_DIR.DS.'temp'.DS.$path,ROOT_DIR.DS.'image'.DS.'users'.DS.$form["id"].DS.$path);
							$sql->Insert(array("tablename"=>'users_images',
												"set"=>array("image_id"=>$lastid,
															"user_id"=>$form["id"])));
							}
						}
